...
/**
  ...
  */
case class Router(accountService: AccountService, transactionRouter: TransactionService)(implicit executionContext: ExecutionContext) {

  private val balanceRoute = new BalanceRouter(accountService).route
  private val transactionRoute = new TransactionRouter(transactionRouter).route
